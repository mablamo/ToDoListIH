...
<ol class="list-group list-group-numbered">
  ...
  v-for="(task) in mySharedTasks" :key="task.id">
    <div class="ms-2 me-auto">
      <div class="fw-bold">{{ task.title }}</div>
      Creada:{{ task.created_at.substr(0, 10) }}
      Para: {{ users[users.findIndex((item) => item.user_id === task.user_target)].user_mail}}
    </div>
    <button class="badge bg-primary rounded-pill" @click="aceptarTask(task)"
...
<ol class="list-group list-group-numbered">
  ...
  v-for="(task) in sharedTasksForMe" :key="task.id">
    <div class="ms-2 me-auto">
      <div class="fw-bold">{{ task.title }}</div>
      Creada:{{ task.created_at.substr(0, 10) }}
      De: {{ users[users.findIndex((item) => item.user_id === task.user_id)].user_mail}}
    </div>
...
export default {
  ...
  computed: {
    ...
    sharedTasksForMe() {
      return this.sharedTasks.filter((elem) => elem.user_target === this.user.id);
    },
  },
  methods: {
